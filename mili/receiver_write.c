...
if (ntohs(sniff_l2->original_source_addr) == from) {
	...
	if (sniff_l3->type == TYPE_UDP){ 
		...
		buffer = (unsigned char*)(sniff_l4_udp) + sizeof(struct layer4_udp) - sizeof(sniff_l4_udp->id);		
	}
}
...
//  unsigned_char *buffer;
	...
	if (recvlen != -1UL) {
	    ...
	    if (ntohs(sniff_l2->original_source_addr) == from) {
			...
			if (sniff_l3->type == TYPE_UDP) {
                		sniff_l4_udp = (struct layer4_udp *) (buffer + sizeof(struct layer2) + sizeof(struct layer3));
				buffer1 = (unsigned char*)(sniff_l4_udp) + sizeof(struct layer4_udp) - sizeof(sniff_l4_udp->id);                	
			}
            }  
